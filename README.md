...
```
3. **В консоли видим 2 вариативных варианта:**
* 1 вариант
```bash
...
2025/12/07 21:44:10 Lost requests: 1  #колличество незавершённых запросов
...
func MakeLinkProcessor(maxGoroutine int) *LinkProcessor {
	...
	lp := &LinkProcessor{
		ch: make(chan struct{}, maxGoroutine),          //создаём буферизированный канал, который будем
...
func (lp *LinkProcessor) LinkChecker(links []string) map[string]string {
	client := &http.Client{                                 //создаём клиента как раз с таймайутом
	...
	for _, l := range links {
		...
		go func(link string, cl *http.Client) {   // если в канале нет места, то это блокирующая операция
			defer lp.wg.Done()
...
* Как только ос отправляет программе `SIGINT(Ctrl+C)` или `SIGTERM(kill)`:
    ...
    4. Если это `SIGTERM`, то:
        ...
        3. Если есть то колличество потеряных запросов принтуется и отправляем в `exitChan`
...
* Так же соответсвуя принципам Инкапсуляции поля `Storage` были сделаны не импортируемыми
